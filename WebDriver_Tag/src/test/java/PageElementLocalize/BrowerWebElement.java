...
public class BrowerWebElement {
	...
	/*
	 * 测试testLink遍历项目，并模糊匹配的代码
	 */
	@Test(groups = ("testLink"))
	public void testLink() throws Exception {
		BeginAndCloseChrome ba = new Tools.BeginAndCloseChrome("file:///E:/ZIXUE/web/testLink.html");
		driver = ba.setUp();

		Select select = new Select(driver.findElement(By.name("select")));

		String srt = "这是第二个选项";
		select.selectByVisibleText(srt);
		assertEquals(srt, select.getFirstSelectedOption().getText());

		List<WebElement> listOption = select.getOptions();
		for (WebElement e : listOption) {
			System.out.println("for:" + e.getText());
			if (e.getText().indexOf("第三个") != -1) {
				System.out.println("if:" + e.getText());
				select.selectByVisibleText(e.getText());
			}
		}

		assertEquals("这是第三个选项", select.getFirstSelectedOption().getText());
	 ...
	 */
	@Test(groups = ("Mooc"))
	public void Mooc() throws Exception {
		BeginAndCloseChrome ba = new Tools.BeginAndCloseChrome("https://www.imooc.com/");
		driver = ba.setUp();

		String username = "";
		String password = "";
		String seting = "//*[@id='header-user-card']/div/div/div[2]/ul[@class='clearfix']/li[4]";

		driver.findElement(By.id("js-signin-btn")).click(); // 点击登录
		driver.findElement(By.name("email")).sendKeys(username); // 输入账号
		driver.findElement(By.name("password")).sendKeys(password); // 输入密码

		driver.findElement(By.xpath("//input[@value='登录']")).click();

		String mainPageHandle = driver.getWindowHandle();// 获取当页句柄
		Actions builder = new Actions(driver);
		builder.moveToElement(driver.findElement(By.id("header-user-card"))).perform();// 获取浮层
		driver.findElement(By.xpath(seting)).click();

		Set<String> handles = driver.getWindowHandles();
		List<String> it = new ArrayList<String>(handles);
		Thread.sleep(3000);
		for (String newPageHandle : it) {
			if (!newPageHandle.equals(mainPageHandle))
				driver.switchTo().window(newPageHandle);
		}

		// 绝对路径定位
		driver.findElement(By.xpath("html/body/div[3]/div/div[1]/div[2]/ul/li[2]/a")).click();
		// 相对路径定位
		driver.findElement(By.xpath("//ul[@class='menu']/li[2]/a")).click();
		// 模糊定位
		driver.findElement(By.xpath("//a[contains(.,'个人信息')]")).click();
		// 遍历li
		List<WebElement> list = driver.findElements(By.xpath("html/body/div[3]/div/div[1]/div[2]/ul/li"));
		for (WebElement e : list) {
			if (e.findElement(By.xpath("./a")).getText().indexOf("个人信息") != -1)
